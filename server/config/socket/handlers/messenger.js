...
} = require('../../../db/db-controllers/messenger-utility');
...
module.exports = (io, socket, context, { onDisconnect }) => {
  ...
  socket.on('disconnect', function () {
    ...
    if (socket.userID) {
      Promise.all([
        simpleUpdateUser(socket.userID, {
          $set: {
            active: false,
        ...
        for (let roomName of relationIds) {
          io.to(`/messenger-user-room/user/${roomName}`).emit(
  ...
  socket.on('ack-call', function (data) {
    if (data.friendID) {
      io.to(`/messenger-user-room/user/${data.friendID}`).emit('ack', {
        from: socket.userID,
      });
    }
  });
  socket.on('end', function (data) {
    if (data.friendID) {
      io.to(`/messenger-user-room/user/${data.friendID}`).emit('end', {
        from: socket.userID,
      });
    }
  });
  socket.on('reject', function (data) {
    if (data.friendID) {
      io.to(`/messenger-user-room/user/${data.friendID}`).emit('reject', {
        from: socket.userID,
      });
    }
  });
  ...
  socket.on('remove-message', function (data) {
    if (data.messageID && data.chatRoomID) {
      ...
      socket.broadcast
        .to(`/messenger-chat-room/chat-room/${data.chatRoomID}`)
        .emit('remove-message', { messageID: data.messageID });
    }
  });
  ...
  socket.on('change-message-reaction', function (data) {
    if (
      ...
      updateMessageReaction(
        ...
        io.to(`/messenger-chat-room/chat-room/${data.chatRoomID}`).emit(
          'change-message-reactions',
          {
  ...
  socket.on('received-message', function (data) {
    if (data.userID && data.chatRoomID && data.messageID) {
      console.log(
        `User ${data.userID} has received message ${data.messageID}!`
      );
  ...
  socket.on('typing-start', function (data) {
    if (data.user && data.chatRoomID) {
      socket.broadcast
        .to(`/messenger-chat-room/chat-room/${data.chatRoomID}`)
        .emit('user-typing', { user: data.user });
    }
  });
  socket.on('typing-done', function (data) {
    if (data.user && data.chatRoomID) {
      socket.broadcast
        .to(`/messenger-chat-room/chat-room/${data.chatRoomID}`)
        .emit('user-typing-done', { user: data.user });
    }
  });
  return io;
};
