...
const {
  getPrivateKey,
  getPublicKey,
} = require('../../authorization/keys/keys');
const { createNewConfirmToken } = require('./confirm-token');
const {
  twilioSmsService,
  nexmoSmsService,
} = require('../../common/sms-service/sms-service');
...
const getAuthenticateUserInitCredentials = (userID) => {
  return User.findOne(
    { _id: ObjectId(userID) },
    '_id contact basic_info joined_at isVerify last_active_at dark_mode search_history followed_posts saved_posts blocked_posts avatar person_blocked'
  )
    .lean()
    .then((data) => {
      if (!data) {
      ...
      return {
        ...data,
        search_history: data.search_history.sort(
          (a, b) =>
...
const shortLogin = ({ _id, password }) => {
  return User.findOne(
    ...
    '_id contact basic_info joined_at chat_settings isVerify chat_settings person_blocked last_active_at dark_mode private_info search_history followed_posts saved_posts blocked_posts avatar'
  )
    .lean()
    .then((data) => {
      if (!data) {
      ...
      return createAuthToken(pick(data, ['_id', 'contact']), getPrivateKey(), {
        ...
        return {
          ...
          user: {
            ...
            search_history: data.search_history.sort(
              (a, b) =>
                new Date(b.search_at).getTime() -
                new Date(a.search_at).getTime()
            ),
          },
        };
      });
...
const login = ({ login_username, password }) => {
  return User.findOne(
    ...
    '_id contact basic_info joined_at followed_posts person_blocked saved_posts blocked_posts isVerify last_active_at chat_settings dark_mode private_info search_history avatar'
  )
    .lean()
    .then((data) => {
      ...
      if (!data.isVerify) {
        return Promise.reject(
          new ApplicationError('account_not_verified', {
            userID: data._id,
...
const verifyChangePasswordToken = ({ token, sessionID }) => {
  return ResetPasswordToken.findOne({
    ...
    .then((data) => {
      ...
      return ResetPasswordToken.findOneAndUpdate(
        { _id: ObjectId(sessionID) },
        { $set: { isVerify: true } },
        { new: true }
      ).lean();
  ...
  return ResetPasswordToken.findOneAndDelete({
    ...
    .then((data) => {
      ...
      return User.findOneAndUpdate(
        ...
        { $set: { 'private_info.password': newPassword.trim() } }
      ).then(() => null);
    });
};
  ...
  return User.findOneAndUpdate(
    ...
    .then(() =>
      User.findOneAndUpdate(
        ...
        { new: true, select: '_id search_history' }
      ).lean()
    )
    .then((data) => {
      return {
...
const updateSearchHistory = (userID, historyID, data) => {
  ...
  return User.findOneAndUpdate(
    { _id: ObjectId(userID), 'search_history._id': ObjectId(historyID) },
    {
      $set: updatedQuery,
    },
    { new: true, select: '_id search_history' }
...
const getUserBasicInfo = (userID, query = {}) => {
  return User.findOne(
    ...
    query.full === 'true'
      ? '_id basic_info avatar contact cover_photo bio joined_at'
      : '_id basic_info avatar cover_photo bio joined_at'
  ).lean();
};
...
const toggleFollowPost = ({ postID, userID }) => {
  return User.findOne({
    ...
    .lean()
    .then((user) => {
      let isPull = user.followed_posts.find(
        (each) => each.post.toString() === postID
      );
      let query = isPull
        ? {
            $pull: {
...
const toggleSavePost = ({ postID, userID }) => {
  return User.findOne({
    ...
    .lean()
    .then((user) => {
      let query = user.saved_posts.find(
        (each) => each._id.toString() === postID
      )
        ? {
            $pull: {
...
const toggleBlockPost = ({ postID, userID }) => {
  return User.findOne({
    ...
    .lean()
    .then((user) => {
      let query = user.blocked_posts.find(
        (each) => each._id.toString() === postID
      )
        ? {
            $pull: {
...
const seenNotifications = ({ userID, notifications }) => {
  return User.findOneAndUpdate(
    ...
    {
      arrayFilters: [
        { 'elem._id': { $in: notifications.map((each) => ObjectId(each)) } },
      ],
      multi: true,
      new: true,
    }
  ...
  return Promise.all([
    ...
    return {
      value: friend.friend_requests.find((each) => each.toString() === userID)
        ...
        : user.friends.find(
            (each) => each.info.toString() === friend._id.toString()
          )
        ? USER_FRIEND_RELATION.FRIEND
        : USER_FRIEND_RELATION.NOT_FRIEND,
    };
  });
};
        ...
        $match: {
          $expr: {
            $eq: [
              ...
              '$friends.info.contact.address.city',
            ],
          },
        },
      },
  ...
  return Promise.all([
    ...
    if (mode === 'birthday') {
      ...
      list = list
        ...
        .filter(
          (each) =>
            each.birthday_countdown < 259200000 && each.birthday_countdown >= 0
        )
    ...
    list = list
      ...
      .map((each) => ({
        ...each,
        same_friends_count: getSameFriends(
          each.friends.info.friends.map((each) => each.info.toString()),
          caller.friends.map((each) => each.info.toString())
        ).length,
        caller_friend_status:
          caller.person_blocked.find(
            (f) => f.toString() === each.friends.info._id.toString()
          ) ||
          each.friends.info.person_blocked.find(
            (f) => f.toString() === caller._id.toString()
          )
            ? 'BLOCKED'
            : !!caller.friends.find(
                (f) => f.info.toString() === each.friends.info._id.toString()
      ...
      .map((each) => ({
        ...
        same_friends_count: each.same_friends_count,
        ...pick(each.friends.info, ['_id', 'avatar', 'basic_info']),
      }));
    return {
      list,
      total,
...
const getUserFriendInvitations = (
  userID,
  { totalOnly = false, skip = 0, limit = 8 }
) => {
  return User.findOne({
    _id: ObjectId(userID),
  })
    ...
    .then((user) => {
      return {
        list: user.friend_requests
          .map((each) => ({
            same_friends_count: getSameFriends(
              each.friends.map((each) => each.info.toString()),
              user.friends.map((each) => each.info.toString())
            ).length,
...
const updateUserPassword = (userID, { newPassword, currentPassword }) => {
  return User.findOneAndUpdate(
    {
      _id: ObjectId(userID),
      'private_info.password': currentPassword.trim(),
    },
    {
      $set: {
        'private_info.password': newPassword.trim(),
      },
    },
    { new: true }
  )
    ...
    .then((data) =>
      data
        ? true
        : Promise.reject(new ApplicationError('wrong_current_password'))
    );
};
...
const checkTargetIsBlocked = (userID, targetID) => {
  return Promise.all([
    ...
    return !!user.person_blocked.find(
      (each) => each.toString() === target._id.toString()
    );
  });
};
...
const getUserBlockedPersons = (userID) => {
  return User.findOne({
    ...
    .then((data) =>
      data.person_blocked.map((each) =>
        pick(each, ['_id', 'basic_info', 'avatar'])
      )
    );
};
