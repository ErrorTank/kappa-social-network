...
const getUserBasicInfo = (userID, query = {}) => {
    return User.findOne(
        ...
        query.full === 'true'
            ? '_id basic_info avatar contact cover_photo bio'
            : '_id basic_info avatar cover_photo bio'
    ).lean();
};
...
const updateUserPassword = (userID, {newPassword, currentPassword}) => {
    return User.findOneAndUpdate({
        ...
        .lean()
        .then((data) => data ? true : Promise.reject(new ApplicationError("wrong_current_password")))
}

const getUserSettings = userID => {
    return User.findOne({
        _id: ObjectId(userID)
    }, "_id notification_settings").lean()
}
