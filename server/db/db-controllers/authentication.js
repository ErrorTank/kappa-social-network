    ...
    return ConfirmToken.findOne({
        ...
        .then(data => {
            ...
            return Promise.all([
                ...
                User.findOneAndUpdate({_id: ObjectId(data.user)}, {$set: {isVerify: true}}, {new: true, select: "_id contact chat_settings basic_info joined_at isVerify last_active_at dark_mode search_history avatar"}).lean()
            ])
        })
        .then(([_, user]) => {
            console.log(user);
