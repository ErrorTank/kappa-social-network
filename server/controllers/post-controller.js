...
} = require('../common/middlewares/post');
const {
  createNewPost,
  getAllPosts,
  updateFilesInPost,
...
module.exports = (db, namespacesIO) => {
    ...
    return createNewPost({
      ...
      .then(([data, shared_post]) => {
        if (req.body.shared_post) {
          namespacesIO.feedPost.socketMap[req.user._id]
        ...
        if (data.policy !== 'PERSONAL') {
          if (
            data.belonged_wall &&
            data.belonged_wall.notification_settings.includes('post_on_wall')
          ) {
            createUserNotification({
              type: 'post_on_wall',
              data: { post: data },
              ...
              namespacesIO.feedPost.io
                .to(`/feed-post-room/user/${data.belonged_wall._id.toString()}`)
                .emit('notify-user', { notification })
            );
          }
          let filesTagged = uniq(
            data.files.reduce(
              (total, cur) => [
                ...total,
                ...cur.tagged
                  .filter((each) =>
                    each.related.notification_settings.includes(
                      'tagged_on_post'
                    )
                  )
                  .map((each) => ({
                    ...
                    file: {
                      ...pick(cur, ['origin_path', 'caption', 'path', 'name']),
                      rootFileID: cur._id,
                    },
                  })),
              ],
          ...
          if (filesTagged.length) {
            for (let u of filesTagged) {
              createUserNotification({
                ...
                namespacesIO.feedPost.io
                  .to(`/feed-post-room/user/${u.userID}`)
                  .emit('notify-user', { notification })
              );
            }
          }
          let postTagged = data.tagged
            .filter((each) =>
              each.notification_settings.includes('tagged_on_post')
            )
            .map((each) => each._id.toString())
            .filter((each) => !filesTagged.find((t) => t.userID === each))
            .filter((each) => each !== req.user._id);
      ...
      return createNewCommentForPost({
        ...
        .then((data) => {
          Promise.all([
            ...
            let fuIds = followedUsers
              .filter((each) =>
                each.notification_settings.includes('comment_on_comment_post')
              )
              .map((each) => each._id.toString())
              .filter((each) => each !== req.user._id);
            let mIds = mentionedUsers
              .filter((each) =>
                each.notification_settings.includes('comment_on_comment_post')
              )
              .map((each) => each._id.toString())
            ...
            for (let u1 of fuIds.filter(
              (each) => !mIds.find((f) => f === each)
            )) {
              createUserNotification({
  ...
  router.post(
    '/create-reply/post/:postID/comment/:commentID',
    authorizationUserMiddleware,
    checkCommentExistedMiddleware,
    (req, res, next) => {
      return createCommentReply({
        ...
        .then((data) => {
          getCommentByReply({ replyID: data._id }).then((comment) => {
            Promise.all([
              getPostByID({ postID: data.post }),
              getMentionsUserByComment(data),
            ]).then(([post, mentionedUsers]) => {
              let mentioned = mentionedUsers
                .filter((each) =>
                  each.notification_settings.includes('comment_on_comment_post')
                )
                .map((each) => each._id.toString())
              ...
              let receivers = uniq(
                comment.replies
                  .filter((each) =>
                    each.from_person.notification_settings.includes(
                      'comment_on_comment_post'
                    )
                  )
                  .map((each) => each.from_person._id.toString())
                  .concat(
                    post.belonged_person.notification_settings.includes(
                      'comment_on_comment_post'
                    )
                      ? post.belonged_person._id.toString()
                      : []
                  )
                  .filter((each) => each !== req.user._id.toString())
  ...
  router.put(
    '/update/post/:postID/file/:fileID',
    authorizationUserMiddleware,
    checkPostExistedMiddleware,
    (req, res, next) => {
      ...
      return toggleFollowPost({
        ...
        .then((data) => {
          if (data.actionType === 'FOLLOWED') {
            namespacesIO.feedPost.socketMap[req.user._id].join(
              `/notification-post-room/post/${req.params.postID}`
            );
          } else {
            namespacesIO.feedPost.socketMap[req.user._id].leave(
              `/notification-post-room/post/${req.params.postID}`
            );
          }
  ...
  router.put(
    '/update-comment/comment/:commentID',
    authorizationUserMiddleware,
    checkCommentExistedMiddleware,
    (req, res, next) => {
      return updateComment({
        ...
        .then((data) => {
          if (data.is_reply) {
            getCommentByReply({ replyID: req.params.commentID }).then(
              (comment) => {
                namespacesIO.feedPost.socketMap[req.user._id]
    ...
    (req, res, next) => {
      return updatePostReaction({
        ...
        .then((data) => {
          ...
          console.log(data);
          if (
            data.belonged_person._id.toString() !== req.user._id &&
            reaction.on &&
            data.belonged_person.notification_settings.includes(
              'react_on_comment_post'
            )
          ) {
            createUserNotification({
              type: 'react_post',
              data: {
                post: data,
                reacted_by: req.user._id,
                reaction: reaction.on,
              },
              userID: data.belonged_person._id,
            }).then((notification) =>
              namespacesIO.feedPost.io
                .to(`/feed-post-room/user/${data.belonged_person._id}`)
                .emit('notify-user', { notification })
            );
          }
  ...
  router.put(
    '/update-reaction/post/:postID/comment/:commentID',
    authorizationUserMiddleware,
    checkCommentExistedMiddleware,
    (req, res, next) => {
      return updatePostCommentReaction({
        ...
        .then((data) => {
          ...
          if (data.from_person._id.toString() !== req.user._id && reaction.on) {
            Promise.all([getPostByID({ postID: req.params.postID })]).then(
              ([post]) => {
                if (
                  data.from_person.notification_settings.includes(
                    'react_on_comment_post'
                  )
                ) {
                  createUserNotification({
                    type: 'react_comment',
                    data: {
                      comment: data.is_reply ? null : data,
                      reply: data.is_reply ? data : null,
                      post,
                      reacted_by: req.user._id,
                      reaction: reaction.on,
                    },
                    ...
                    namespacesIO.feedPost.io
                      .to(`/feed-post-room/user/${data.from_person._id}`)
                      .emit('notify-user', { notification })
                  );
                }
              }
  ...
  router.get(
    '/reaction/post/:postID/reaction_key/:reactionKey',
    authorizationUserMiddleware,
    (req, res, next) => {
