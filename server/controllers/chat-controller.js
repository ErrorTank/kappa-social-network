...
const {
    ...
    seenMessages,
    getChatRoomBrief,
} = require('../db/db-controllers/chat-room');
...
module.exports = (db, namespacesIO) => {
    ...
    router.get(
        ...
        (req, res, next) => {
            return getGroupChatRoomInvolvesByKeyword(
    ...
    router.get(
        '/:chatRoomID/nicknames',
        authorizationUserMiddleware,
        (req, res, next) => {
            return getChatRoomNicknames(req.params.chatRoomID)
                .then((data) => {
                    return res.status(200).json(data);
                })
                .catch((err) => next(err));
            ...
            return createNewMessage({
                ...
                .then((data) => {
                    ...
                    getChatRoomBrief(req.params.chatRoomID).then((cr) => {
                        for (let user of cr.involve_person.filter(
                            (each) => each.related.toString() !== result.sentBy._id.toString()
                        )) {
                            console.log(
    ...
    router.put(
        '/:chatRoomID/messages/update-to-sent',
        authorizationUserMiddleware,
        (req, res, next) => {
            return updateSavedMessagesToSent(
                req.params.chatRoomID,
    ...
    router.put(
        '/:chatRoomID/messages/seen-messages',
        authorizationUserMiddleware,
        (req, res, next) => {
            return Promise.all([
                seenMessages(
    ...
    router.get(
        '/:chatRoomID/get-messages',
        authorizationUserMiddleware,
        (req, res, next) => {
            return getChatRoomMessages(req.params.chatRoomID, {
                take: req.query.take,
                skip: req.query.skip,
            })
                .then((data) => {
    ...
    router.put(
        '/:chatRoomID/user/:userID/nickname',
        authorizationUserMiddleware,
        (req, res, next) => {
                    ...
                    return createNewMessage({
                        ...
                        namespacesIO.messenger.io
                            ...
                            .emit('update-nicknames', {
                                data: data
                                    .toObject()
                                    .map((each) => ({...each, related: each.related._id})),
                            });
                        return res.status(200).json(data);
                    });
                })
    ...
    router.put(
        '/:chatRoomID/default-emoji',
        authorizationUserMiddleware,
        (req, res, next) => {
            return updateChatRoomDefaultEmoji(req.params.chatRoomID, req.body.value)
                .then((data) => {
                    return createNewMessage({
                        chatRoomID: req.params.chatRoomID,
                        value: {
