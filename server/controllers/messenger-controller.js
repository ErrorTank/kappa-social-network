...
module.exports = (db, namespacesIO) => {
  router.get(
    ...
    (req, res, next) => {
      return Promise.all([
        simpleUpdateUser(req.user._id, {
          $set: {
            active: req.params.status === 'true',
            last_active_at: new Date().getTime(),
          },
        }),
        getAllUserActiveRelations(req.user._id),
      ])
        .then(([_, data]) => {
          ...
          for (let roomName of relationIds) {
            ...
            namespacesIO.messenger.io
              .to(`/messenger-user-room/user/${roomName}`)
              .emit('change-contact-status', {
                userID: req.user._id,
                active: req.params.status === 'true',
              });
          }
  ...
  router.get(
    '/bubble/user/:userID/brief',
    authorizationUserMiddleware,
    (req, res, next) => {
      return getUserBubbleChatBrief(req.user._id, req.params.userID)
        .then((data) => {
          return res.status(200).json(data);
        })
        .catch((err) => next(err));
  ...
  router.get(
    '/user/:userID/chat-rooms',
    authorizationUserMiddleware,
    (req, res, next) => {
      return getUserChatRooms(req.user._id, req.query.skip)
        .then((data) => {
          return res.status(200).json(data);
        })
        .catch((err) => next(err));
  ...
  router.get(
    '/chat-room/user/:userID/brief',
    authorizationUserMiddleware,
    (req, res, next) => {
      return getUserChatRoomBrief(req.user._id, req.params.userID)
        .then((data) => {
          return res.status(200).json(data);
        })
        .catch((err) => next(err));
