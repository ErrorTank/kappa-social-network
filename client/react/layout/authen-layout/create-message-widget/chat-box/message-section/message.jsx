...
import {
  getActiveReaction,
  haveReaction,
} from '../../../../../../common/utils/messenger-utils';
...
export const HyperlinkWrapper = (props) =>
  props.links.length ? (
    <a
      href={props.links.length ? props.links[0] : ''}
      target={'_blank'}
...
export class Message extends Component {
  ...
  renderMessageState = (state) => {
    switch (state) {
      case MessageState.CACHED:
        return <span className='msg-state'></span>;
  ...
  onClickFile = () => {
    ...
    if (!isImageFile(name)) {
      ...
      utilityApi.downloadFile(origin_path, name).finally(() => {
  ...
  render() {
    ...
    return message.special !== MESSAGE_TYPES.CASUAL ? (
      ...
      <div
        className={classnames('chat-message', position, {
          owned: isOwned,
      ...
      >
        <div className='upper-panel'>
          ...
          {!message.is_deleted && isOwned && !message.call_info && (
            <MessageAction
          ...
          <div className={classnames('content-wrapper', { owned: isOwned })}>
            ...
            {message.call_info ? (
              <div className='call-info-message'>
                <div className='upper-info'>
                  <div className='icon-wrapper'>
                    {message.call_info.call_type === 'VOICE' && (
                      <i className='fas fa-phone-alt'></i>
                    )}
                    {message.call_info.call_type === 'VIDEO' && (
                      <i className='fas fa-video'></i>
                    )}
                  </div>
                  <div className='call-info'>
                    <div className='call-info-type'>
                      {message.call_info.call_status === 'MISSED' &&
                        'Cuộc gọi nhỡ'}
                      {message.call_info.call_status === 'FINISH' &&
                        'Cuộc gọi kết thúc'}
                    </div>
                    <div className='call-info-report'>
                      {message.call_info.call_status === 'MISSED' &&
                        moment(message.created_at).format('hh:mm A')}
                      {message.call_info.call_status === 'FINISH' &&
                        (() => {
                          let formatDuration = transformSecondsToTime(
                            message.call_info.duration
                          );
                          return `${
                            formatDuration.hours > 0
                              ? formatDuration.hours + ' giờ '
                              : ''
                          }${
                            formatDuration.minutes > 0
                              ? formatDuration.minutes + ' phút '
                              : ''
                          }${formatDuration.seconds + ' giây'}`;
                        })()}
                    </div>
                  </div>
                </div>
                <button
                  ...
                  onClick={() => this.props.recall(message.call_info.call_type)}
                >
                  Gọi lại
                </button>
              </div>
              ...
              <>
                    ...
                    <div className='reply-title'>
                      ...
                      {isOwned ? (
                        ...
                        <span className='high-light'>
                          {message.sentBy.basic_info.username}
                        </span>
                      )}{' '}
                      phản hồi{' '}
                      {message.sentBy._id !== message.reply_for.sentBy._id && (
                        <span>
                          tới{' '}
                          {userID === message.reply_for.sentBy._id ? (
                            'bạn'
                          ) : (
                            <span className='high-light'>
                              {message.reply_for.sentBy.basic_info.username}
                            </span>
                          )}
                        </span>
                      )}
                ...
                <div
                  className={classnames('message-renderable-content', {
                    emoji: message.emoji,
                    file: message.file,
                    owned: isOwned,
                            ...
                            {product && (
                              <div className='product'>
                                <div className='brief-picture'>
                                  <img src={product.files[0].path} alt='' />
                                </div>
                                ...
                                <div className='brief-info'>
                                  ...
                                  <div className='bi-price'>
                                    {product.price &&
                                      numberToMoney(product.price.toString())}
                                  </div>
                                </div>
                              </div>
                            )}
  ...
  render() {
    ...
    return (
      <>
        ...
        <div
          className={classnames('message-action', {
            show: show || showReaction,
          })}
        >
          {!isReverse ? (
            <div className='message-action-wrapper'>
              ...
              >
                <ClickOutside
                  onClickOut={() =>
                    showReaction && this.setState({ showReaction: false })
                  }
                >
                  <div
                    className='action'
                    onClick={() =>
                      this.setState({ showReaction: !showReaction })
                    }
                  >
