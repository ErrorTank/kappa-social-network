...
class ReceiverInfo extends Component {
  constructor(props) {
    ...
    let receiverID = props.chatRoom.involve_person.find(
...
export class MessageSection extends Component {
  constructor(props) {
    ...
    this.io.on("user-typing-done", ({ user }) => {
      let isBottom = this.isBottom();
      this.setState(
        { typing: this.state.typing.filter((each) => each._id !== user._id) },
        () => {
          isBottom && setTimeout(() => this.scrollToBottom());
        }
      );
  ...
  getMessagePositionState = (messages, index) => {
    ...
    if (
      previous?.sentBy._id !== current.sentBy._id &&
      after?.sentBy._id !== current.sentBy._id
    ) {
      return "single";
    }
    if (
  ...
  changeReaction = (config, messageID) => {
    this.io.emit("change-message-reaction", {
      chatRoomID: this.props.chatRoomID,
      messageID,
            ...
            if (!(firstMessage && firstMessage.is_init)) {
              ...
              this.loadMessages(this.props.chatRoomID).then(() => {
                setTimeout(() => {
                  ...
                  elemClone.scrollTop =
                    elemClone.scrollHeight - oldScrollHeight;
                });
              });
            }
          }}
