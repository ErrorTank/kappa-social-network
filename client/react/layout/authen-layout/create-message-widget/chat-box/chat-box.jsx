...
import {
  ...
  MessageSection,
} from './message-section/message-section';
import {
  messageUtilities,
  MessageUtilities,
} from './message-utilities/message-utilities';
...
import {
  CALL_TYPES,
  callServices,
} from '../../../../../common/call-services/call-services';
...
export class ChatBox extends KComponent {
  constructor(props) {
    ...
    messengerApi.getUserChatRoomBrief(props.userID).then(({ chat_room }) => {
      ...
      this.io.on(
        'new-message',
        ({ message, senderID, forceUpdate = false }) => {
          let scrollToLatest = messagesContainerUtilities.createScrollLatest();
    ...
    this.onUnmount(
      this.messageState.onChange((nextState, oldState) => {
        this.forceUpdate();
      })
    );
  }
  ...
  startVideoCall = () => {
    if (!callServices.isCalling()) {
      ...
      openVideoModal({
        ...
        chatRoomID: this.state.chat_room_brief?._id,
        multiple: false,
        callTo: this.state.chat_room_brief.involve_person.filter(
          (each) => each.related !== userInfo.getState()._id
        )[0].related,
        onMinimize: (reOpen) => {
  ...
  handleSubmitChat = (chatState) => {
    ...
    let fileMessages = chatState.files.map((each) =>
      this.createTempMessage({ file: each, needUploadFile: true })
    );
    ...
    if (newMessage) {
      chatApi
        ...
        .then((newServerMessage) => {
          ...
          let replaceIndex = msgs.findIndex(
            (each) => each._id === newServerMessage.oldID
          );
  ...
  changeAllSavedMassageToSent = (messages) => {
    let userID = userInfo.getState()._id;
    let savedMessage = messages.filter(
      (each) => each.sentBy._id !== userID && each.state === 'SAVED'
    );
    if (savedMessage.length) {
      chatApi.changeSavedMessagesToSent(
        this.state.chat_room_brief._id,
        savedMessage
      );
    }
  ...
  loadMessages = (chatRoomID) => {
    ...
    return chatApi
      .getChatRoomMessages(chatRoomID, { skip: oldMsgs.length })
      .then((messages) => {
  ...
  emitSeenMessageEvent = () => {
    ...
    let unseenMessages = messages.filter(
      (each) =>
        each.sentBy._id !== userID &&
        each.state === 'SENT' &&
        !each.seenBy.find((seen) => seen._id === userID)
    );
                ...
                <MessageBoxLayout
                  ...
                  renderHeader={() => (
                    <div className='chat-box-header message-widget-header'>
                      ...
                      {userInfo && <ToggleMinimize userInfo={userInfo} />}
                    </div>
                  )}
                  renderBody={() => (
