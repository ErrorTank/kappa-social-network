...
import {
  CALL_STATUS,
  MediaCallLayout,
} from "../media-call-layout/media-call-layout";
...
export class VoiceCallWidget extends Component {
  ...
  render() {
    let { microphone, webcam, shareScreen } = this.state;
    ...
    return (
      <div
        className={classnames("voice-call-modal", {
          active:
            callStatus === CALL_STATUS.CALLING && type === CALL_TYPES.VIDEO,
        })}
      >
          ...
          <div className="voice-modal-content">
            {callStatus !== CALL_STATUS.CALLING ? (
              ...
              <>
                ...
                <VideoPlayer
                  className="peerVideo"
                  hide={type === CALL_TYPES.VOICE}
                  getRef={(peerVideo) => (this.peerVideo = peerVideo)}
                />
                <VideoPlayer
                  className="localVideo"
                  hide={type === CALL_TYPES.VOICE}
                  getRef={(localVideo) => (this.localVideo = localVideo)}
                  muted={true}
                />
              </>
            )}
          ...
          <div className="footer-actions">
            {actions.map(
              (
                ...
                <Tooltip text={() => toolTip} position={"top"} key={i}>
                  <div
                    className={classnames("footer-action", className, {
                      active: isActive,
                      disabled,
                    })}
                    onClick={onClick}
...
export class VoiceCallModal extends Component {
  constructor(props) {
    ...
    userApi
      .getUserBasicInfo(props.config.callTo)
  ...
  render() {
    ...
    return (
      <MediaCallLayout
        ...
        onFinish={(callStatus) => {
          config.onFinish?.({
            callStatus: callStatus ? "MISSED" : "FINISH",
...
const VideoPlayer = React.memo(
  ({ className, hide, autoPlay = true, muted = false, getRef }) => {
    console.log("dsadsa4232");
    return (
      <video
        className={classnames(className, { hide })}
        ref={(video) => getRef(video)}
        autoPlay={autoPlay}
        muted={muted}
      />
    );
  ...
  (prevProps, nextProps) => {
    return (
      ...
      prevProps.getRef !== nextProps.getRef
    );
  }
);
