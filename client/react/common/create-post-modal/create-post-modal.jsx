...
export const createPostModal = {
  open(config) {
    const modal = modals.openModal({
      content: <CreatePostModal {...config} onClose={(r) => modal.close(r)} />,
    });
    return modal.result;
  },
};
...
class CreatePostModal extends Component {
  ...
  submit = () => {
    ...
    Promise.all(files.map((each) => this.uploadSingleFile(each))).then(
      (newFiles) => {
        ...
        this.props.isEdit
          ? postApi
              .updatePost(this.props.data._id, submittedData)
              .then((data) => this.props.onClose(data))
          : postApi.createNewPost(submittedData).then((data) => {
              return userFollowedPosts
                .setState(userFollowedPosts.getState().concat(data._id))
                .then(() => this.props.onClose(data));
            });
      }
    );
  };
  getInputRawContent = () => {
    return transformEditorState(
  ...
  render() {
    let { onClose, isEdit, isShare, postID, shareMarketplace } = this.props;
      ...
      },
      {
        ...
        component: (
          <FilesDisplay
            ...
            onRemove={(file) => {
              let newFiles = this.state.files.filter((each) =>
                file.path
                  ? file.path !== each.path
                  : each.fileID !== file.fileID
              );
              this.setState({
                files: newFiles,
        ...
        component: (
          <FileConfig
            ...
            onChange={(file) => {
              ...
              this.setState({
                files: newFiles,
                stepIndex: newFiles.length === 1 ? 0 : 1,
              });
              let newTagged = mergeArray(
                this.state.tagged,
                file.tagged.map((each) => each.related),
