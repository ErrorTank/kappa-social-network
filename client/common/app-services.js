...
const initializeAuthenticateUser = ({ userInfo: uInfo, authToken }) => {
  ...
  return Promise.all([
    ...
    feedPostIO
      .connect({ token: authToken || authenCache.getAuthen() })
      .then((feedPostIO) => {
        feedPostIO.emit("join-own-room", { userID: uInfo._id });

        feedPostIO.on("notify-user", ({ notification }) => {
          feedPostIO.emit("seen-notification", {
            notificationID: notification._id,
            userID: uInfo._id,
          });
          bottomNotification.actions.push({
            content: <PostNotification notification={notification} />,
          });
        });
      }),
    messengerIO
...
const clearAuthenticateUserSession = () => {
  return messengerApi.sendActiveStatusToAllRelations(false).then(() => {
...
export { initializeAuthenticateUser, clearAuthenticateUserSession };
